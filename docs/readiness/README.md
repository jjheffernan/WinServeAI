...
- **Overall project maturity:** **3.3/5** (`mvp-partial`) — average of 14 modules, excluding `ui` (Phase 2 only).
- **Phase alignment:** Phase 1 engine + G/H resident IPC are in place. Phase 2 tray (`winserve-tray`) and Phase 3 Inno/stage-release shipped in the draft PR stack; **A2** Windows+GGUF smoke and ISCC CI remain open. Operator docs cover tray + IPC (I1).
